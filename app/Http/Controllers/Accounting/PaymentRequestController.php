...
class PaymentRequestController extends Controller
{
    function showPrintCheque(Request $request){
        ...
        if(isset($data['prid'])){
            ...
            $selectQuery = PaymentRequest::where('status','=','RELEASED')
                ...
                ->first();
            $pdf = PDF::loadView('accounting-department.payment-request.pdf.print-cheque', array('data' => $selectQuery))
            ...
            return redirect(route('accounting-payment-request-list'));
        }
    }
        ...
        if(isset($data['prid'])){
            ...
            if($selectQuery){
                if(in_array($selectQuery->status,$allowEdit)) {
                    ...
                    return view('accounting-department.payment-request.details')
        ...
        if(isset($data['prid'])){
            ...
            if($selectQuery) {
                if (in_array($selectQuery->status, $allowEdit)) {
                    ...
                    return view('accounting-department.payment-request.update')
                        ->with('admin_menu', 'ACCOUNTING')
                        ->with('admin_sub_menu', 'CREATE-PR')
                        ->with('payment_request', $selectQuery)
                    ...
                    return redirect(route('accounting-payment-request-details', ['prid' => $data['prid']]));
                }
            }else{
                Session::flash('success',0);
                Session::flash('message','Unable to find payment request data. Please try again');
    ...
    function showIndex(Request $request){
        ...
        $data = $request->all();
        $type = 'CASH';
        $admin_menu = 'CASH-LIST';
        if(isset($data['type']) && $data['type'] == 'cheques'){
            $type = 'CHEQUES';
            $admin_menu = 'CHEQUE-LIST';
        }
        return view('accounting-department.payment-request.index')
            ->with('admin_menu','ACCOUNTING')
            ->with('admin_sub_menu',$admin_menu)
            ->with('user',$user);
    }
    function showCreate(){
        $user = Auth::user();
        $selectDepartmentQuery = Department::all();
        $selectAccountTitleQuery = AccountingTitle::all();
        return view('accounting-department.payment-request.create')
            ->with('admin_menu','ACCOUNTING')
            ->with('admin_sub_menu','CREATE-PR')
            ->with('departments',$selectDepartmentQuery)
            ->with('accountTitles',$selectAccountTitleQuery)
    ...
    function showFunctions(Request $request,$postMode = null){
        ...
        if ($request->ajax()){
            ...
            elseif($postMode == 'released-payment-request-list'){
                $types = ['CASH','PETTY-CASH'];
                if($data['type'] == 'CHEQUES'){
                    $types = ['CHEQUE'];
                }
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->addColumn('pr_number_details', function($selectQuery){
                        $returnValue = '';
                        $enc_payment_request_id = encryptor('encrypt',$selectQuery->id);
                        $paymentDetails = route('accounting-payment-request-update',['prid' => $enc_payment_request_id]);
                        if($selectQuery->category == 'OFFICE'){
            ...
            elseif($postMode == 'cancelled-payment-request-list'){
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->editColumn('pr_number', function($selectQuery){
                        $returnValue = '';
                        $enc_payment_request_id = encryptor('encrypt',$selectQuery->id);
                        $paymentDetails = route('accounting-payment-request-details',['prid' => $enc_payment_request_id]);
                        if($selectQuery->category == 'OFFICE'){
                            $returnValue .= '<span class="badge badge-primary">'.$selectQuery->designated_department.'</span> | ';
                        }
                        $returnValue .= $selectQuery->pr_number.' ';
                ...
                if($validator->fails()){
                    ...
                    if($data['payment_type'] == 'CASH'){
                        ...
                        if(isset($data['partials'])) {
                            $attributes['cheque_types'] = 'Cheque Types';
                            $attributes['cheque_dates'] = 'Cheque Dates';
                        ...
                        $resultHtml .='
                            <tr id="po-row-'.$enc_purchase_order_id.'">
                                <td>
                                    ...
                                    <input type="hidden" name="po[]" value="'.$purchaseOrder->po_number.'" />
                    ...
                    if($selectQuery){
                        if($data['status'] == 'PENDING'){
                            ...
                            try {
                                ...
                                return redirect(route('accounting-payment-request-update',['prid' => $enc_payment_request_id]));
                            }catch (QueryException $exception) {
                                DB::rollback();
                                Session::flash('success', 0);
                                Session::flash('message', $exception->errorInfo[2]);
                            ...
                            if($validator->fails()){
                                ...
                                if($isPartialValidated == false){
                                    ...
                                    try {
                                        ...
                                        return redirect(route('accounting-payment-request-details',['prid' => $enc_payment_request_id]));
                                    }catch (QueryException $exception) {
                                        DB::rollback();
                                        Session::flash('success', 0);
                                        Session::flash('message', $exception->errorInfo[2]);
