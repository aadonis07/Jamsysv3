...
class PurchaseOrderController extends Controller
{
    function pdfPODetails(Request $request){
        ...
        if(isset($data['po']) && !empty($data['po'])){
            ...
            $selectQuery = PurchaseOrder::with('supplier')
                ...
                ->first();
            $pdf = PDF::loadView('it-department.purchasing.pdf.po-details', array('data' => $selectQuery))
    ...
    function showPOdetails(Request $request){
        ...
        if(isset($data['poid']) && !empty($data['poid'])){
            ...
            if($selectQuery){
                ...
                return view('it-department.purchasing.details')
    ...
    function showUpdatePO(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectQuery){
                $selectDepartmentQuery = Department::all();
                return view('it-department.purchasing.update-p-o')
                    ->with('admin_menu','PURCHASING')
                    ->with('admin_sub_menu','LIST')
    ...
    function showIndex(){
        $user = Auth::user();
        return view('it-department.purchasing.index')
            ->with('admin_menu','PURCHASING')
            ->with('admin_sub_menu','LIST')
            ->with('user',$user);
    }
    function showCreatePO(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectQuery){
                ...
                $selectDepartmentQuery = Department::all();
                if($isExist){
                    return redirect(route('supplier-update-p-o',['sid' => $data['sid']]));
                }else{
                    return view('it-department.purchasing.create-p-o')
                        ->with('admin_menu','PURCHASING')
    ...
    function showSuppliers(){
        $user = Auth::user();
        return view('it-department.purchasing.supplier-list')
            ->with('admin_menu','PURCHASING')
            ->with('admin_sub_menu','CREATE-P-O')
            ->with('user',$user);
    }
        ...
        if ($request->ajax()){
            if($postMode == 'quotation-products'){
                ...
                $selectQuery = QuotationProduct::with('fitout_products')->with('product')
                        ->where('type','=','SUPPLY') // for now supply na muna. Kapag naka designate na ito sa per department mababago to.
                    ...
                    ->editColumn('product_name', function($selectQuery) use($data){
                        ...
                        $returnValue = '
                            <div class="custom-control custom-radio">
                                <input  required onChange=$("#quotation-product-key").val("'.$quotation_product_id.'") value="'.$selectQuery->product_name.'" type="radio" class="custom-control-input quotation-products" name="quotation_product_name" id="quotation-product-'.$quotation_product_id.'">
                                <label class="custom-control-label" for="quotation-product-'.$quotation_product_id.'">DESIGNATE QTY</label>
                            </div>
                        ';
            ...
            elseif($postMode == 'for-approval-po-list') {
                ...
                return Datatables::eloquent($selectQuery)
                                ...
                                ->addColumn('actions', function($selectQuery){
                                    ...
                                    $poUpdate = route('p-o-details',['poid' => $enc_purchase_order_id]);
                                    $poPrint = route('pdf-p-o-details',['po' => $selectQuery->po_number]);
            ...
            elseif($postMode == 'in-progress-po-list') {
                ...
                return Datatables::eloquent($selectQuery)
                        ...
                        ->addColumn('actions', function($selectQuery){
                            ...
                            $poUpdate = route('supplier-update-p-o',['sid' => $enc_supplier_id]);
                        ...
                        if($selectQuery->type == 'RAW' || $selectQuery->type == 'SPECIAL-ITEM' ){
                            ...
                            $htmlData .= '
                                ...
                                <td>
                                    ...
                                    <hr class="m-0">
                                    <text class="text-primary">Date Created: [ SAVE P.O FIRST ]</text>
                                    <hr class="m-0">
            ...
            elseif($postMode == 'supplier-raw-products') {
                ...
                return Datatables::eloquent($selectQuery)
                        ...
                        ->editColumn('code', function($selectQuery) {
                            ...
                            $returnValue .= '<hr class="m-0">';
                            $returnValue .= '<text title="ERP data" class="text-primary"><span class="badge badge-primary">'.$selectQuery->product->type.'</span>: '.$selectQuery->product->product_name.'</text>';
                            return $returnValue;
                        })
                        ->editColumn('price', function($selectQuery) {
                            $returnValue = '&#8369; '.number_format($selectQuery->price,2);
                            $returnValue .= '<hr class="m-0">';
                            $returnValue .= '<text class="text-primary">SYSTEM: &#8369; '.number_format($selectQuery->product->base_price,2).'</text>';
                            return $returnValue;
                        })
                        ->smart(true)
                        ->escapeColumns([])
            ...
            elseif($postMode == 'supplier-supply-products') {
                ...
                return Datatables::eloquent($selectQuery)
                        ...
                        ->editColumn('code', function($selectQuery) {
                            $returnValue = $selectQuery->code;
                            $returnValue .= '<hr class="m-0">';
                            $returnValue .= '<text title="ERP data" class="text-primary"><span class="badge badge-primary">'.$selectQuery->variant->parent->type.'</span>: '.$selectQuery->variant->parent->product_name.'</text>';
                            $returnValue .= '<hr class="m-0">';
                            $returnValue .= '<text title="ERP data" class="text-primary"><span class="badge badge-primary">VARIANT</span>: '.$selectQuery->variant->product_name.'</text>';
                            return $returnValue;
                        })
                        ...
                        ->editColumn('price', function($selectQuery) {
                            $returnValue = '&#8369; '.number_format($selectQuery->price,2);
                            $returnValue .= '<hr class="m-0">';
                            $returnValue .= '<text class="text-primary">SYSTEM: &#8369; '.number_format($selectQuery->variant->base_price,2).'</text>';
                            return $returnValue;
                        })
                        ->smart(true)
                        ->escapeColumns([])
                ...
                return Datatables::eloquent($selectQuery)
                    ->addColumn('actions', function($selectQuery){
                        ...
                        $isExist = PurchaseOrder::where('supplier_id','=',$selectQuery->id)
                            ...
                            ->first();
                        if($isExist){
                            $poUrl = route('supplier-update-p-o', ['sid' => $enc_supplier_id]);
                            $returnValue = '<a href="' . $poUrl . '" class="btn btn-primary  btn-sm waves-effect waves-themed mb-1" data-toggle="tooltip" data-placement="top" title="Update" data-original-title="UPDATE">
                                            Update P.O
                                        </a>&nbsp;';
                        }else{
                            if($selectQuery->supplyProducts->count() > 0 ) { // for supply supplier
                                $poUrl = route('supplier-create-p-o', ['sid' => $enc_supplier_id]);
                                $returnValue = '<a href="' . $poUrl . '" class="btn btn-info  btn-sm waves-effect waves-themed mb-1" data-toggle="tooltip" data-placement="top" title="Update" data-original-title="UPDATE">
                                            Create P.O
                                        </a>&nbsp;';
                            }
                            elseif($selectQuery->rawProducts->count() > 0) { // for raw supplier
                                $poUrl = route('supplier-create-p-o', ['sid' => $enc_supplier_id]);
                                $returnValue = '<a href="' . $poUrl . '" class="btn btn-info  btn-sm waves-effect waves-themed mb-1" data-toggle="tooltip" data-placement="top" title="Update" data-original-title="UPDATE">
                                            Create P.O
                                            </a>&nbsp;';
                                ...
                                $returnValue = '<b class="text-info">No products</b>';
                            }
                        }
            ...
            if($postMode == 'update-price-product'){
                $supplier_id = encryptor('decrypt',$data['supplier_key']);
            ...
            elseif($postMode == 'po-move-to-pending'){
                ...
                if($selectQuery){
                    ...
                    if($selectQuery->save()){
                        ...
                        return redirect(route('purchasing-list'));
                    }else{
                        Session::flash('success', 0);
            ...
            elseif($postMode == 'add-qty-product'){
                ...
                if(isset($data['purchase_order'])){
                    ...
                    if($selectQuery){
                        ...
                        foreach ($selectQuery as $detail){
                            if($data['type'] == 'QUOTATION'){
                                ...
                                if( ( $name == $detail->name ) && $data['department'] == $detail->department ){
                                    $is_validated = 0;
                                    break;
                                }
                            }
                    ...
                    if(Cart::session('PUR-'.$supplier_id)->get($supplier_product_id)) {
                        ...
                        foreach ($details as $detail){
                            if($data['type'] != 'QUOTATION') {
                                if ( ( $detail['type'] == $data['type'] ) && ( $detail['department'] == $data['department'] ) ) {
                                    $is_validated = 0;
                                    break;
