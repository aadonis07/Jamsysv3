...
class PurchaseOrderController extends Controller
{
    function pdfPODetails(Request $request){
        ...
        if(isset($data['po']) && !empty($data['po'])){
            ...
            $selectQuery = PurchaseOrder::with('supplier')
                ...
                ->first();
            $pdf = PDF::loadView('purchasing-raw-department.purchasing.pdf.po-details', array('data' => $selectQuery))
    ...
    function showPOdetails(Request $request){
        ...
        if(isset($data['poid']) && !empty($data['poid'])){
            ...
            if($selectQuery){
                ...
                return view('purchasing-raw-department.purchasing.details')
    ...
    function showUpdatePO(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectQuery){
                $selectDepartmentQuery = Department::all();
                return view('purchasing-raw-department.purchasing.update-p-o')
                    ->with('admin_menu','PURCHASING')
                    ->with('admin_sub_menu','LIST')
    ...
    function showIndex(){
        $user = Auth::user();
        return view('purchasing-raw-department.purchasing.index')
            ->with('admin_menu','PURCHASING')
            ->with('admin_sub_menu','LIST')
            ->with('user',$user);
    }
    function showCreatePO(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectQuery){
                ...
                $selectDepartmentQuery = Department::all();
                if($isExist){
                    return redirect(route('purchasing-raw-supplier-update-p-o',['sid' => $data['sid']]));
                }else{
                    return view('purchasing-raw-department.purchasing.create-p-o')
                        ->with('admin_menu','PURCHASING')
    ...
    function showSuppliers(){
        $user = Auth::user();
        return view('purchasing-raw-department.purchasing.supplier-list')
            ->with('admin_menu','PURCHASING')
            ->with('admin_sub_menu','CREATE-P-O')
            ->with('user',$user);
    }
        ...
        if ($request->ajax()){
            if($postMode == 'quotation-products'){
                ...
                $selectQuery = QuotationProduct::with('fitout_products')->with('product')
                    ->whereIn('type',['RAW','CUSTOMIZED','COMBINATION','SPECIAL-ITEM']) // for now supply na muna. Kapag naka designate na ito sa per department mababago to.
            ...
            elseif($postMode == 'for-approval-po-list') {
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->addColumn('actions', function($selectQuery){
                        ...
                        $poUpdate = route('purchasing-raw-p-o-details',['poid' => $enc_purchase_order_id]);
                        $poPrint = route('purchasing-raw-pdf-p-o-details',['po' => $selectQuery->po_number]);
            ...
            elseif($postMode == 'approved-po-list') {
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->addColumn('actions', function($selectQuery){
                        $enc_purchase_order_id = encryptor('encrypt',$selectQuery->id);
                        $poUpdate = route('purchasing-raw-p-o-details',['poid' => $enc_purchase_order_id]);
                        $poPrint = route('purchasing-raw-p-o-details',['po' => $selectQuery->po_number]);
                        $returnValue = '<a class="btn btn-primary btn-xs" title="View Details" href="'.$poUpdate.'"><span class="fas fa-eye "></span></a>';
                        $returnValue .= '&nbsp;<a target="_blank" class="btn btn-warning btn-xs" title="Print Details [ Internal use ]" href="'.$poPrint.'"><span class="fas fa-print "></span></a>';
                        return $returnValue;
                    })
            ...
            elseif($postMode == 'in-progress-po-list') {
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->addColumn('actions', function($selectQuery){
                        ...
                        $poUpdate = route('purchasing-raw-supplier-update-p-o',['sid' => $enc_supplier_id]);
                    ...
                    if($isExist){
                        ...
                        try {
                            DB::transaction(function () use ($data,$supplier_id,$user,$product_details,$total_ordered) {
                                ...
                                $po_number = generatePoNumber('RAW'); // default encapse SUPPLY
                                ...
                                $insertQuery->type = 'RAW'; //default is supply
                ...
                return Datatables::eloquent($selectQuery)
                    ->addColumn('actions', function($selectQuery){
                        ...
                        $isExist = PurchaseOrder::where('supplier_id','=',$selectQuery->id)
                            ...
                            ->first();
                        if($isExist){
                            $poUrl = route('purchasing-raw-supplier-update-p-o', ['sid' => $enc_supplier_id]);
                            ...
                            if($selectQuery->rawProducts->count() > 0) { // for raw supplier
                                $poUrl = route('purchasing-raw-supplier-create-p-o', ['sid' => $enc_supplier_id]);
                                $returnValue = '<a  title="CREATE P.O" href="' . $poUrl . '" class="btn btn-info  btn-sm waves-effect waves-themed mb-1" data-toggle="tooltip" data-placement="top" title="Update" data-original-title="UPDATE">
            ...
            if($postMode == 'po-update-status'){
                ...
                if($updateQuery){
                    ...
                    if($updateQuery->save()){
                        ...
                        return redirect(route('purchasing-raw-list'));
                    }
                    else{
                        Session::flash('success', 0);
