...
class SupplierController extends Controller
{
    function showSuppliers(Request $request){
        ...
        $selectIndustryQuery = Industry::where('is_active', '=', 1)
            ...
            ->get();
        $regions = showRegions();
        return view('purchasing-raw-department.suppliers.index')
    ...
    function showSupplierProducts(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectSupplier){
                return view('purchasing-raw-department.suppliers.supplier-products')
                    ->with('admin_menu','SUPPLIERS')
                    ->with('admin_sub_menu','')
        ...
        if(isset($data['id']) && !empty($data['id'])){
            ...
            if($supplier) {
                ...
                $selectCitiesQuery = City::where([['province_id', '=', $supplier->province_id],
                    ...
                    ->get();
                return view('purchasing-raw-department.suppliers.page-load.supplier-details')
        ...
        if ($request->ajax()){
            if($postMode == 'supplier-raw-products'){
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->editColumn('product.product_name', function($selectQuery){
                        $returnValue = $selectQuery->product->product_name;
                        $returnValue .= '<hr class="m-0">';
                        $returnValue .= '<text class="text-primary">TYPE: <b>'.$selectQuery->product->type.'</b></text>';
                        return $returnValue;
                    })
                    ...
                    ->editColumn('price', function($selectQuery){
                        ...
                        $returnValue .= '<hr class="m-0">';
                        $returnValue .= '<text class="text-primary"><b>SYSTEM: &#8369; '.number_format($selectQuery->product->base_price,2).' </b></text>';
                        return $returnValue;
                    })
                    ->smart(true)
                    ->escapeColumns([])
            ...
            elseif($postMode == 'raw-and-special-item-product-list') {
            ...
            elseif($postMode == 'supplier-list') {
				...
				$department_id  = 7; //PUR-RM
                $selectQuery = Supplier::where('department_id','=',$department_id)
                                    ->orderBy('created_at'); 
                return Datatables::eloquent($selectQuery)
                    ->addColumn('actions', function($selectQuery){
                        $enc_supplier_id = encryptor('encrypt',$selectQuery->id);
                        $productsUrl = route('purchasing-raw-supplier-products',['sid'=>$enc_supplier_id]);
            ...
            elseif($postMode == 'fetch-provinces') {
                ...
                $resultHtml = '';
                $selectProvincesQuery = Province::where([['region_id', '=', $region_id], ['is_enable','=',true]])
                    ->orderBy('description', 'ASC')
                    ->get();
            ...
            elseif($postMode == 'fetch-cities') {
                ...
                $resultHtml = '';
                $selectCitiesQuery = City::where([['province_id', '=', $province_id], ['is_enable','=',true]])
                    ->orderBy('city_name', 'ASC')
                    ->get();
            ...
            elseif($postMode == 'add-suppliers') {
                $attributes = [
                    ...
                    'select-category' => 'Supplier Category',
                    'supplier-contact-person' => 'Supplier Contact Person',
                    'supplier-email' => 'Supplier Email',
                    'select-region' => 'Supplier Region',
                    'select-city' => 'Supplier City',
                    'supplier-tin-number' => 'Supplier Tin Number',
                    'select-industry' => 'Supplier Industry',
                    'supplier-contact-number' => 'Supplier Contact Number',
                    'select-vat' => 'Supplier Vat',
                    'select-province' => 'Supplier Province',
                    'supplier-complete-address' => 'Supplier Complete Address',
                    'supplier-payment-type' => 'Supplier Payment Type',
                    'supplier-payment-term' => 'Supplier Payment Term',
                    'supplier-remarks' => 'Supplier Remarks',
                ];
                $rules = [
                    'supplier-name' => 'required|unique:suppliers,name,NULL,id,archive,0',
                    'supplier-code' => 'required|unique:suppliers,code,NULL,id,archive,0',
                    ...
                    'supplier-payment-term' => 'required',
//                    'supplier-remarks' => 'Supplier Remarks',
                ];
                $validator = Validator::make($data,$rules,[],$attributes);
                if($validator->fails()){
