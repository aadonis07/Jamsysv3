...
class PaymentRequestController extends Controller
{
    function showPrintCheque(Request $request){
        ...
        if(isset($data['prid'])){
            ...
            $selectQuery = PaymentRequest::where('status','=','RELEASED')
                ...
                ->first();
            $pdf = PDF::loadView('purchasing-supply-department.payment-request.pdf.print-cheque', array('data' => $selectQuery))
            ...
            return redirect(route('purchasing-supply-payment-request-list'));
        }
    }
        ...
        if(isset($data['prid'])){
            ...
            if($selectQuery){
                if(in_array($selectQuery->status,$allowEdit)) {
                    return view('purchasing-supply-department.payment-request.details')
        ...
        if(isset($data['prid'])){
            ...
            if($selectQuery) {
                if (in_array($selectQuery->status, $allowEdit)) {
                    ...
                    return view('purchasing-supply-department.payment-request.update')
                        ->with('admin_menu', 'ACCOUNTING')
                        ->with('admin_sub_menu', 'CREATE-PR')
                        ->with('payment_request', $selectQuery)
                    ...
                    return redirect(route('purchasing-supply-payment-request-details', ['prid' => $data['prid']]));
                }
            }else{
                Session::flash('success',0);
                Session::flash('message','Unable to find payment request data. Please try again');
    ...
    function showIndex(){
        $user = Auth::user();
        return view('purchasing-supply-department.payment-request.index')
            ->with('admin_menu','ACCOUNTING')
    ...
    function showCreate(){
        ...
        $selectDepartmentQuery = Department::all();
        $selectAccountTitleQuery = AccountingTitle::all();
        return view('purchasing-supply-department.payment-request.create')
            ->with('admin_menu','ACCOUNTING')
            ->with('admin_sub_menu','CREATE-PR')
            ->with('departments',$selectDepartmentQuery)
            ->with('accountTitles',$selectAccountTitleQuery)
            ...
            elseif($postMode == 'released-payment-request-list'){
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->addColumn('pr_number_details', function($selectQuery){
                        $returnValue = '';
                        $enc_payment_request_id = encryptor('encrypt',$selectQuery->id);
                        $paymentDetails = route('purchasing-supply-payment-request-update',['prid' => $enc_payment_request_id]);
                        if($selectQuery->category == 'OFFICE'){
            ...
            elseif($postMode == 'cancelled-payment-request-list'){
                ...
                return Datatables::eloquent($selectQuery)
                    ...
                    ->editColumn('pr_number', function($selectQuery){
                        $returnValue = '';
                        $enc_payment_request_id = encryptor('encrypt',$selectQuery->id);
                        $paymentDetails = route('purchasing-supply-payment-request-details',['prid' => $enc_payment_request_id]);
                        if($selectQuery->category == 'OFFICE'){
                            $returnValue .= '<span class="badge badge-primary">'.$selectQuery->designated_department.'</span> | ';
                        }
                        $returnValue .= $selectQuery->pr_number.' ';
                        ...
                        if($isPartialValidated == false){
                            ...
                            try {
                                DB::transaction(function () use ($user,$data,$pr_number) {
                                    ...
                                    $insertQuery->status = 'R-DEPARTMENT';
                                    ...
                                    if(isset($data['partials'])){
                                        ...
                                        foreach($data['partials'] as $index=>$partial){
                                            ...
                                            $partialPayment['status'] = 'R-DEPARTMENT';
                                    ...
                                    elseif($data['payment_type'] == 'CHEQUE'){
                                        ...
                                        if(isset($data['partials'])){
                                            ...
                                            foreach($data['partials'] as $index=>$partial){
                                                ...
                                                $partialPayment['amount'] = $partial;
                                                $partialPayment['status'] = $updateQuery->status;
                                                $partialPayment['purpose'] = $data['purposes'][$index];
                                                $partialPayment['cheque_type'] = $data['cheque_types'][$index];
                                                $partialPayment['cheque_date'] = $data['cheque_dates'][$index];
                                                $partialPayment['created_by'] = $user->id;
...
//                                    }
            ...
            if($postMode == 'update-payment-request-status'){
                    ...
                    if($selectQuery){
                        if($data['status'] == 'R-DEPARTMENT'){
                            ...
                            try {
                                ...
                                Session::flash('message', $selectQuery->pr_number.' is now reverted to R-DEPARTMENT section.');
                                return redirect(route('purchasing-supply-payment-request-update',['prid' => $enc_payment_request_id]));
                            }catch (QueryException $exception) {
                                DB::rollback();
                                Session::flash('success', 0);
                            ...
                            if($validator->fails()){
                                ...
                                if($isPartialValidated == false){
                                    ...
                                    try {
                                        ...
                                        return redirect(route('purchasing-supply-payment-request-details',['prid' => $enc_payment_request_id]));
                                    }catch (QueryException $exception) {
                                        DB::rollback();
                                        Session::flash('success', 0);
                                        Session::flash('message', $exception->errorInfo[2]);
