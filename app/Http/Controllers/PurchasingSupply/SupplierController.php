...
class SupplierController extends Controller
{
    function showSuppliers(Request $request){
        ...
        $selectIndustryQuery = Industry::where('is_active', '=', 1)
            ...
            ->get();
        $regions = showRegions();
        return view('purchasing-supply-department.suppliers.index')
    ...
    function showSupplierProducts(Request $request){
        ...
        if(isset($data['sid']) && !empty($data['sid'])){
            ...
            if($selectSupplier){
                return view('purchasing-supply-department.suppliers.supplier-products')
                    ->with('admin_menu','SUPPLIERS')
                    ->with('admin_sub_menu','')
        ...
        if(isset($data['id']) && !empty($data['id'])){
            ...
            if($supplier) {
                ...
                $selectCitiesQuery = City::where([['province_id', '=', $supplier->province_id],
                    ...
                    ->get();
                return view('purchasing-supply-department.suppliers.page-load.supplier-details')
    ...
    function showFunctions(Request $request,$postMode = null){
        ...
        $user = Auth::user();
        if ($request->ajax()){
            if($postMode == 'supplier-supply-products'){
