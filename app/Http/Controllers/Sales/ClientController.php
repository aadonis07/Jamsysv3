...
class ClientController extends Controller
{
    function showClients(Request $request){
        ...
        $selectBusinessStyleQuery = BusinessStyle::orderBy('name', 'ASC')
                                ->get();
        return view('sales-department.clients.index')
        ...
        if(isset($data['id']) && !empty($data['id'])){
            ...
            if($client) {
                ...
                $selectCitiesQuery = City::where('province_id', '=', $client->province_id)
                                                ...
                                                ->get();
                $selectBarangayQuery = Barangay::where('city_id', '=', $client->city_id)
                                                ->where('is_enable','=',true)
                                                ->orderBy('barangay_description', 'ASC')
                                                ->get();
                return view('sales-department.clients.page-load.client-details')
                                        ->with('client', $client)
                                        ->with('regions', $regions)
    ...
    function showCompanyBranches(Request $request){
        ...
        if(isset($data['cid']) && !empty($data['cid'])){
            ...
            $regions = showRegions();
            return view('sales-department.clients.company-branches')
                ->with('admin_menu','CLIENTS')
    ...
    function companyBranchContent(Request $request){
        ...
        if(isset($data['id']) && !empty($data['id'])){
            ...
            $selectBarangayQuery = Barangay::where('city_id', '=', $company_branch->city_id)
                                                ->where('is_enable','=',true)
                                                ->orderBy('barangay_description', 'ASC')
                                                ->get();
            $select_regions = '';
            ...
            foreach($selectBarangayQuery as $barangay) {
                ...
                $current_barangay = '';
                if($barangay->id == $company_branch->barangay_id) { $current_barangay = 'selected'; }
                $select_barangays .= '<option value="'.$enc_barangay_id.'" '.$current_barangay.'>'.$barangay->barangay_description.'</option>';
            }
            $resultHtml = '
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="text" class="form-control" required name="contact-person-update" id="contact-person-update" value="'.$company_branch->contact_person.'">
                </div>
                <div class="form-group mb-2">
                    <label>Position :</label>
                    <input type="text" class="form-control" required name="position-update" id="position-update" value="'.$company_branch->position.'">
                </div>
                <div class="form-group mb-2">
                    <label>Contact Number :</span></label>
                    <input type="text" class="bootstrap-tagsinput" name="contact-number-update" id="contact-number-update" value="'.$company_branch->contact_numbers.'">
                </div>
                <div class="form-group mb-2">
    ...
    function showFunctions(Request $request,$postMode = null){
        ...
        if ($request->ajax()){
            if($postMode == 'client-list') {
                $selectQuery = Client::with('companyBranches')
                                        ->with('user.employee')
                        ...
                        return Datatables::eloquent($selectQuery)
                                    ->addColumn('actions', function($selectQuery){
                                        ...
                                        $branchLink = route('sales-client-company-branches',['cid' => $enc_client_id]);
                                    ...
                                    ->editColumn('name', function($selectQuery) {
                                        ...
                                        $count_branches = 0;
                                        if(!empty($selectQuery->companyBranches)) {
                                            $count_branches = count($selectQuery->companyBranches);
                                        }
                                        $returnValue ='<span class="badge badge-primary">'.$client_type.'</span> | ';
                                        $returnValue .= $selectQuery->name.' <span class="badge badge-primary">'.$count_branches.'</span>';
                                    ...
                                    ->editColumn('agents', function($selectQuery) {
                                        $first_name = "";
                                        $last_name = "";
                                        $agent_name = "-";
                                        if(!empty($selectQuery->user->employee)) {
                                            $first_name = $selectQuery->user->employee->first_name;
                                            $last_name = $selectQuery->user->employee->last_name;
                                            $agent_name = $first_name.' '.$last_name;
                                        }
                                        $returnValue = $agent_name;
                                        return $returnValue;
                                    })
            ...
            if($postMode == 'add-clients') {
                ...
                if($validator->fails()){
                    ...
                    $insertClientQuery->barangay_id = $barangay_id;
            ...
            elseif($postMode == 'update-clients') {
                ...
                if($validator->fails()){
                    ...
                    $updateClientQuery->barangay_id = $barangay_id;
            ...
            elseif($postMode == 'add-company-branches'){
                ...
                if($validator->fails()){
                    ...
                    $insertCompanyBranchQuery->contact_person = trim($data['contact-person']);
                    $insertCompanyBranchQuery->position = trim($data['position']);
                    $insertCompanyBranchQuery->contact_numbers = trim($data['contact-number']);
                    ...
                    $insertCompanyBranchQuery->barangay_id = $barangay_id;
            ...
            elseif($postMode == 'update-company-branches'){
                ...
                $attributes = [
                    ...
                    'contact-number-update' => 'Contact Number',
                ...
                $rules = [
                    ...
                    'contact-person-update' => 'required',
                    'position-update' => 'required',
                    'contact-number-update' => 'required',
                    'select-region-update' => 'required',
                    'select-province-update' => 'required',
                    'select-city-update' => 'required',
                    'select-barangay-update' => 'required',
                ...
                if($validator->fails()){
                    ...
                    $updateCompanyBranchQuery->contact_person = trim($data['contact-person-update']);
                    $updateCompanyBranchQuery->position = trim($data['position-update']);
                    $updateCompanyBranchQuery->contact_numbers = trim($data['contact-number-update']);
                    ...
                    $updateCompanyBranchQuery->barangay_id = $barangay_id;
