...
class QuotationController extends Controller
{
    public function create(){
        ...
        return view('sales-department.quotations.create')
    ...
    public function list(){
        ...
        return view('sales-department.quotations.list')
             ->with('admin_menu','QUOTATION')
    ...
    function quotationPreview(Request $request){
        $data = $request->all();
        $user = Auth::user();
        return view('sales-department.quotations.preview.quotation_preview')->with('user',$user);
    }
    function jobrequestProductList(Request $request){
        ...
        if(isset($dec_id)){
            $products = QuotationProduct::with('product')->where('quotation_id','=',$dec_id)
                                        ...
                                        ->get();
            if(count($products)==0){
                $returnHtml = '<div class="form-group" align="center"><h1>All your products is in Job Request.</h1></div>';
            }else{
                $product_content = '';
                $count=0;
        ...
        if ($request->ajax()){
            if($postMode=='quotation-list-serverside'){
                if($user->position_id == 7){
                    $selectQuery = Quotation::with('client')->with('job_request')->where('status','=',$data['status'])
                                ->where('user_id','=',$user->id)->whereNull('hold_date')
                                ->orderBy('created_at','DESC');
                }else{
                    $selectQuery = Quotation::with('client')->with('job_request')->where('status','=',$data['status'])
                                    ->where('team_id','=',$user->team_id)->whereNull('hold_date')
                                    ->orderBy('created_at','DESC');
                }
                ...
                ->editColumn('created_at', function($selectQuery) use($user) {
                    ...
                    if(!empty($selectQuery->date_moved)){
                        $returnHTml .= '<b class="text-danger">DATE MOVED :</b>'.date('F d,Y ',strtotime($selectQuery->date_moved));
                    }
                ...
                ->addColumn('actions', function($selectQuery) use($user) {
                    $returnHtml = '<div align="center">';
                        ...
                        if($selectQuery->status == 'PENDING' || $selectQuery->status == 'REJECTED' || (!empty($selectQuery->hold_date))){
                        ...
                        if(!empty($selectQuery->date_moved)){
                            $returnHtml .= '<a class="btn btn-icon btn-outline-white waves-effect rounded-circle mr-1 request-reject-quotation" style="border-color:black;" data-status="'.$selectQuery->status.'" data-id="'.encryptor('encrypt',$selectQuery->id).'" title="Request For Rejection">
                ...
                ->addColumn('actions', function($selectQuery) use($user) {
                    $returnHtml = '<div align="center">';
                        ...
                        if(!empty($selectQuery->date_accounting_approved)){
                            if($selectQuery->status!='REJECTED'||$selectQuery->status!='CANCELLED'||$selectQuery->status!='R-REJECT'){
                                $returnHtml .= '<a class="btn btn-icon btn-standard btn-outline-info waves-effect rounded-circle mr-1" href="'.route('sales-delivery-create',['id'=>encryptor('encrypt',$selectQuery->id)]).'" title="Request For Delivery">
                ...
                return $computation;
            }elseif($postMode=='request-hold'){
                $id = encryptor('decrypt',$data['id']);
                $holdQuotation = Quotation::find($id);
                $holdQuotation->hold_request_date = date('Y-m-d');
                $holdQuotation->updated_at = getDatetimeNow();
                if($holdQuotation->save()){
                    return $holdQuotation->status;
                }
            }elseif($postMode=='request-reject'){
                $id = encryptor('decrypt',$data['id']);
                $holdQuotation = Quotation::find($id);
                $status = $holdQuotation->status;
                $holdQuotation->status = 'R-REJECT';
                $holdQuotation->previous_status = $status;
                $holdQuotation->hold_request_date = null;
                $holdQuotation->updated_at = getDatetimeNow();
                if($holdQuotation->save()){
                    return $status;
                }
            ...
            if($postMode=='quotation-information'){
                ...
                if($data['actionMode']=='view'){
                    ...
                    $banks = Bank::all();
                    return view('sales-department.quotations.view')
                    ...
                    return view('sales-department.quotations.update')
                         ->with('work_nature',$work_nature)
                         ->with('roles',$roles)
                         ->with('delivery_modes',$delivery_modes)
                         ->with('clients',$clients)
                ...
                if($validator->fails()){
                    ...
                    try {
                        ...
                        if(empty($updateQuery->is_requested_si)){
                            if($data['request-si']=='true'){
                                $updateQuery->is_requested_si = getDatetimeNow();
                            }
                        }
                        if(!empty($data['commission-solution'])){
                            $updateQuery->request_commission = $data['request-commi'];
                            $updateQuery->final_commission = $data['final-commission'];
                            $updateQuery->commission_type = $data['commission-type'];
                            $updateQuery->commission_formula = $data['commission-solution'];
                            $updateQuery->note = $data['formula-legend'];
                        }
                        if($updateQuery->save()){
                            DB::commit();
                            $count = 0;
                ...
                if($validator->fails()){
                    ...
                    if($data['type']=='FIT-OUT'){
                        ...
                        $check_products = checkRedundant($product_id,$selectQuery->id);
            ...
            }elseif($postMode=='move-quotation'){
                $id = encryptor('decrypt',$data['qID']);
                $selectQuery = Quotation::find($id);
                $payment_modes = array(
                    'CASH'=>'Cash',
                    'ONLINE'=>'Online',
                    'CHECK'=>'Check',
                ...
                $attributes = [
                    ...
                    'processing-period'=>'Processsing Period',
                ...
                if($validator->fails()){
                    ...
                    DB::beginTransaction();
                    try {
                        DB::transaction(function () use ($data, $user,$selectQuery,$payment_modes,$vat_types,$banks,$payment_terms,$delivery_modes) {
                            if(!empty($data['payment-terms'])){
                                $term_id = $data['payment-terms'];
                            }else{
                                $term_id = $selectQuery->quotation_term_id;
                            }
                            $term = QuotationTerm::find($term_id);
                            $insertCollection = new Collection();
                            $insertCollection->quotation_id = $selectQuery->id;
                            $insertCollection->client_id = $selectQuery->client_id;
                            $insertCollection->agent_id =  $selectQuery->agent->id;
                            $insertCollection->contract_amount = $selectQuery->grand_total;
                            $insertCollection->terms = $term->name;
                            $insertCollection->tin_number = $data['tin-number'];
                            if(!empty($data['client-po-number'])){
                                $insertCollection->client_po_number = $data['client-po-number'];
                            }
                            $insertCollection->created_by = $user->id; 
                            $insertCollection->updated_by = $user->id;
                            $insertCollection->created_at = getDatetimeNow();
                            $insertCollection->updated_at = getDatetimeNow();
                            if($insertCollection->save()){
                                DB::commit();
                                $selectQuery->status = 'FOR-APPROVAL';
                                $selectQuery->quotation_term_id = $term_id;
                                $selectQuery->date_moved = date('Y-m-d');
                                $selectQuery->vat_type = $data['vat-type'];
                                $selectQuery->updated_by = $user->id;
                                $selectQuery->updated_at = getDatetimeNow();
                                if($selectQuery->save()){
                                    DB::commit();
                                    $updateClient = Client::find($selectQuery->client->id);
                                    $updateClient->is_verified = 1;
                                    if($updateClient->save()){
                                        DB::commit();
                                        if((!empty($data['amount']))||(!empty($data['with-held-amount']))){
                                            $collectionDetail = new CollectionDetail();
                                            $collectionDetail->collection_id = $insertCollection->id;
                                            $collectionDetail->collection_date = $data['collection-date'];
                                            $collectionDetail->type = 'DOWNPAYMENT';
                                            if(!empty($data['amount'])){
                                                $collectionDetail->amount_paid = $data['amount'];
                                            }
                                            $collectionDetail->payment_type = $data['payment-mode'];
                                            if(!empty($data['bank'])){
                                                $collectionDetail->bank_id = $data['bank'];
                                            }
                                            if(!empty($data['check-number'])){
                                                $collectionDetail->check_number = $data['check-number'];
                                            }
                                            if(!empty($data['check-date'])){
                                                $collectionDetail->check_date = $data['check-date'];
                                            }
                                            if(!empty($data['with-held-amount'])){
                                                $collectionDetail->ewt_amount = $data['with-held-amount'];
                                            }
                                            $collectionDetail->status = 'UNVERIFIED';
                                            $collectionDetail->created_by = $user->id;
                                            $collectionDetail->updated_by = $user->id;
                                            $collectionDetail->created_at = getDatetimeNow();
                                            $collectionDetail->updated_at = getDatetimeNow();
                                            if($collectionDetail->save()){
                                                DB::commit();
                                                $destination_terms = 'assets/files/quotation_num/';
                                                $filename_terms = $selectQuery->quote_number;
                                                $isExist = isExistFile($destination_terms.''.$filename_terms); 
                                                ...
                                                if($result['success'] == true){
                                                    Session::flash('success', 1);
                                                    Session::flash('message', 'Successfully Moved!');
                                                }else{
                                                    DB::rollback();
                                                    Session::flash('success', 0);
