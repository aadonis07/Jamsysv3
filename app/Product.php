...
class Product extends Model implements Auditable
{
    ...
    public function parentWithCategory(){
        return $this->belongsTo('App\Product','parent_id','id')->with('subCategoryWithCategory')->whereNull('parent_id');
    }
    public function parent(){
        return $this->belongsTo('App\Product','parent_id','id')->whereNull('parent_id');
    }
    public function supplierProduct(){
        return $this->hasMany('App\SupplierProduct','product_id','id');
    }
    public function default_product(){
        return $this->belongsTo('App\Product','id','parent_id')->where('is_default','=','1');
    }
}
