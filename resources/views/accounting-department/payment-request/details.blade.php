                ...
                <div class="row p-3">
                    @if($payment_request->status == 'RELEASED' && $payment_request->type == 'CHEQUE' && $payment_request->is_partial == false)
                        <div class="col-md-12 text-right">
                            <a target="_blank" href="{{ route('accounting-payment-request-pdf-print-cheque',['prid'=> $enc_payment_request_id]) }}" disabled class="btn btn-warning btn-sm"><span class="fas fa-print"></span> | PRINT CHEQUE</a>
                            <hr class="mt-2 mb-2">
                        </div>
                    @endif
                    <div class="col-md-7">
                                            ...
                                            <tr>
                                                ...
                                                <td width="60%">
                                                    ...
                                                    @elseif($partial->status == 'RELEASED')
                                                        | <a target="_blank" href="{{ route('accounting-payment-request-pdf-print-cheque',['prid'=> $enc_payment_request_id,'pprid' => $enc_partial_payment_request_id ]) }}" disabled class="btn btn-warning btn-xs"><span class="fas fa-print"></span> | PRINT CHEQUE</a>
                                                    @endif
                                                    <hr class="mt-1 mb-1">
                                        ...
                                        <table class="table">
                                            ...
                                            @foreach($payment_request->details as $index=>$detail)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <form method="post" id="add-liquidation-{{ $detail->id }}-form" action="{{ route('accounting-payment-request-functions',['id' => 'ewt-liquidation']) }}">
                                                            @csrf()
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0 mt-0">
                    @if($payment_request->is_partial == true)
                        <form method="post" id="release-payment-form" onsubmit="$('#release-payment-id').attr('disabled',true)"  action="{{ route('accounting-payment-request-functions',['id' => 'update-partial-payment-request-status']) }}">
                            @csrf()
                            <div class="row">
                                <input type="hidden" name="payment_request_key" value="{{ $enc_payment_request_id }}"/>
                    ...
                    @else
                        <form method="post" id="release-payment-form" onsubmit="$('#release-payment-id').attr('disabled',true)"  action="{{ route('accounting-payment-request-functions',['id' => 'update-payment-request-status']) }}">
                            @csrf()
                            <div class="row">
                                <input type="hidden" name="key" value="{{ $enc_payment_request_id }}"/>
                                <input type="hidden" name="status" value="RELEASED"/>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body pt-0 mt-0">
                    @if($payment_request->status == 'FOR-APPROVAL' && $payment_request->is_partial == true)
                        <form method="post"  id="update-partial-payment-status-form" onsubmit="$('#approved-pr-btn').attr('disabled',true)" id="update-payment-request-status" action="{{ route('accounting-payment-request-functions',['id' => 'update-partial-payment-request-status']) }}">
                            @csrf()
                            <div class="row">
    ...
    <div class="modal fade" id="released-pr-partial-modal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0 mt-0">
                    <form method="post" id="release-partial-payment-form" onsubmit="$('#released-partial-payment-id').attr('disabled',true)"  action="{{ route('accounting-payment-request-functions',['id' => 'update-partial-payment-request-status']) }}">
                        @csrf()
                        <div class="row">
                            <input type="hidden" name="payment_request_key" value="{{ $enc_payment_request_id }}"/>
