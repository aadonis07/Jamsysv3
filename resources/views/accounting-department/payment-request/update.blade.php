                                            ...
                                            <tr id="partial-row-{{ $enc_payment_request_partial_id }}">
                                                <td>
                                                    ...
                                                    @if($payment_request->status == 'PENDING')
                                                        <hr class="m-0 mb-1">
                                                        <div class="custom-control custom-checkbox">
                                                            <input  required value="{{ $enc_payment_request_partial_id }}"  id="partial-checkbox-{{ $enc_payment_request_partial_id }}"  type="checkbox" class="custom-control-input" name="selected_partials[]" >
                                                            <label class="custom-control-label" for="partial-checkbox-{{ $enc_payment_request_partial_id }}">FOR APPROVAL</label>
                                                        </div>
                                                    @endif
                                                </td>
                                                <td>
...
{{--                                        <th width="3%">Action</th>--}}
                    ...
                    <div class="col-md-6 text-danger">
                        <p>* Note: Moving this request to <b>FOR APPROVAL</b> status will be no longer to modify / edit. Please double check the inputs.  </p>
                    </div>
                    <div class="col-md-6 text-right">
                        <form method="post"  id="update-payment-status-form" onsubmit="$('#update-payment-status-btn').attr('disabled',true)" id="update-payment-request-status" action="{{ route('accounting-payment-request-functions',['id' => 'update-payment-request-status']) }}">
                            @csrf()
                            <input type="hidden" name="status" value="FOR-APPROVAL"/>
                            <input type="hidden" name="account_title" id="update-account-title" value="{{ $payment_request->account_title_id }}"/>
                            <input type="hidden" name="particular" id="update-particular" value="{{ $payment_request->particular_id }}"/>
                            <input type="hidden" name="key" value="{{ $enc_payment_request_id }}"/>
                            <div id="selected_partials" style="display:none">

                            </div>
                        </form>
                        @if($payment_request->department_id == $user->department_id)
                        ...
                        @else
                            <button type="button" onClick="updatePaymentStatus('REJECTED')"  id="cancel-payment-status-btn" title="REJECT PR" disabled class="btn btn-danger move-to-status-btn"><span class="fas fa-times "></span> | REJECT</button>
                        @endif
                        <button type="button" onClick="updatePaymentStatus('FOR-APPROVAL')"  id="update-payment-status-btn" title="Update status to FOR APPROVAL" disabled class="btn btn-info move-to-status-btn">FOR APPROVAL | <span class="fas fa-arrow-right "></span></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0 mt-0">
                    <div class="row">
                        <div class="col-md-12">
                            <form method="post"  id="cancel-payment-status-form" onsubmit="$('#cancel-pr-btn').attr('disabled',true)" id="update-payment-request-status" action="{{ route('accounting-payment-request-functions',['id' => 'update-payment-request-status']) }}">
                                @csrf()
                                <div class="form-group">
    ...
    <script>
        ...
        function employees(){
            ...
            $('#employees-table').DataTable({
                ...
                "ajax":{
                    url :"{{ route('accounting-payment-request-functions',['id' => 'employee-list']) }}", // json datasource
        ...
        function payees(){
            $("#payee").select2({
                ajax:{
                    type: "POST",
                    url: "{{ route('accounting-payment-request-functions',['id' => 'payee-list']) }}",
                    //dataType: 'json',
                    delay: 250,
        ...
        function addParticipant(btn){
            ...
            $.ajax({
                type: "POST",
                url: "{{route('accounting-payment-request-functions',['id' => 'add-particulars'])}}",
                data: formData,
                contentType: false,
                processData: false,
        ...
        function suppliers(){
            ...
            $('#suppliers-table').DataTable({
                ...
                "lengthMenu": [[50, 100, 150, 250], [50, 100, 150, 250]],
                "ajax":{
                    url :"{{ route('accounting-payment-request-functions',['id' => 'supplier-list']) }}", // json datasource
                    type: "POST",  // method  , by default get
                    error: function(result){  // error handling
                        $('#err').html(JSON.stringify(result));
                    }
        ...
        function clients(){
            ...
            $('#clients-table').DataTable({
                ...
                "lengthMenu": [[50, 100, 150, 250], [50, 100, 150, 250]],
                "ajax":{
                    url :"{{ route('accounting-payment-request-functions',['id' => 'clients-list']) }}", // json datasource
                    type: "POST",  // method  , by default get
                    error: function(result){  // error handling
                        $('#err').html(JSON.stringify(result));
                    }
        ...
        function getParticulars(key){
            ...
            $.ajax({
                type: "POST",
                url: "{{route('accounting-payment-request-functions',['id' => 'get-particulars'])}}",
                data: formData,
                contentType: false,
                processData: false,
                success: function (result) {
        ...
        function addPrSupplier(key){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('accounting-payment-request-functions',['id' => 'supplier-validated-po']) }}", // STATUS: APPROVED P. STATUS: FOR-REQUEST
                data: formData,
                contentType: false,
                processData: false,
                success: function (result) {
        ...
        function selectClientQuotation(key,tempKey){
            ...
            $('#client-quotations-table').DataTable({
                ...
                "lengthMenu": [[50, 100, 150, 250], [50, 100, 150, 250]],
                "ajax":{
                    url :"{{ route('accounting-payment-request-functions',['id' => 'client-quotations-list']) }}", // json datasource
                    type: "POST",  // method  , by default get
                    data : { client: key },
                    error: function(result){  // error handling
                        $('#err').html(JSON.stringify(result));
        ...
        function updateRequest(){
            ...
            confirm_message('Update Payment Request','Are you sure you want to  update this request ?' , function (confirmed) {
                    if(confirmed) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{route('accounting-payment-request-functions',['id' => 'update-payment-request'])}}",
                            data: formData,
                            contentType: false,
                            processData: false,
                            success: function (result) {
        ...
        function updatePaymentStatus(status){
            if(status == 'REJECTED' || status == 'CANCELLED' ){
                ...
                $('#selected_partials').empty();
                confirm_message('Update Payment Request Status','Are you sure you want to  update this request status ?' , function (confirmed) {
                    if(confirmed) {
                        $("input[name='selected_partials[]']").each(function(index){
                            if ($(this).prop("checked")) {
                                $('#selected_partials').append('' +
                                    '<input name="selected_partials[]" value="' + this.value + '">' +
                                '');
                            }
                            // all partials
                            $('#selected_partials').append('' +
                                '<input name="partials[]" value="' + this.value + '">' +
                            '');
                        });
        ...
        function createPartial(btn){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('accounting-payment-request-functions',['id' => 'create-partial-payment-request']) }}",
                data: formData,
                contentType: false,
                processData: false,
                success: function (result){
