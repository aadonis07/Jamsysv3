...
<div class="modal fade" id="create-erp-modal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
			<form method="post" id="create-erp-form" onsubmit="addbtn.disabled = true;"  action="{{ route('employee-functions', ['id' => 'create-erp']) }}">
                @csrf()
...
<script>
 $(function(){
            $('#dt-employees-regular').DataTable( {
                ...
                "serverSide": true,
                "ajax":{
                    url :"{{ route('employee-functions',['id' => 'employee-list-serverside']) }}",
                    type: "POST",  
					data: {status:"REGULAR"},
					"pageLength": 100,
                   "processing": true,
			...
			$('#dt-employees-separated').DataTable( {
                ...
                "serverSide": true,
                "ajax":{
                    url :"{{ route('employee-functions',['id' => 'separated-employee-list-serverside']) }}",
                    type: "POST",  
					"pageLength": 100,
                   "processing": true,
                   "serverSide": true,
...
$(document).ready(function(index){
	$(document).on('click','.employee_account',function(){
		...
		$('#employee-account-content').html('' +
            ...
            '</div>'+
            '');
		var path = "{{route('employee-account')}}?id="+id;
			...
			if($.trim(username)){
				...
				if($.trim(password)){
					...
					$('input[name="password"]').addClass('is-valid');
						...
						$.ajax({
							type: "POST",
							url: "{{ route('employee-functions', ['id' => 'add-account']) }}",
							data: formData,
							CrossDomain:true,
							contentType: !1,
							processData: !1,
	...
	$(document).on('click','.show-password',function(){
		...
		Swal.fire({
			...
			preConfirm: (login) => {
				...
				$.ajax({
					type: "POST",
					url: "{{ route('employee-functions', ['id' => 'validate-password']) }}",
					data: formData,
					CrossDomain:true,
					contentType: !1,
					processData: !1,
				...
				$.ajax({
					...
					success: function(success) {
						if(success.id!=0){
							...
							'</div>'+
							'</td>');
							var path = "{{route('employee-update-account')}}?id="+id+"&&type="+type;
							$('#tab'+id).load(path);
						}else{
							Swal.fire({
							type: 'error',
			...
			if($.trim(username)){
				...
				if($.trim(password)){
					...
					$('#password'+id).addClass('is-valid');
						$.ajax({
							type: "POST",
							url: "{{ route('employee-functions', ['id' => 'update-account']) }}",
							data: formData,
							CrossDomain:true,
							contentType: !1,
							processData: !1,
							success: function(e) {
								var path = "{{route('employee-account')}}?id="+e;
								$('#employee-account-content').load(path,function(){
									$('select[name="account-type"]').select2({
										placeholder:"Select Account Type",
										allowClear: true,
	...
	$(document).on('click','.employee-info',function(){
		...
		$('#employee-info-content').html('' +
            ...
            '</div>'+
            '');
		var path = "{{route('employee-info')}}?id="+id;
	...
	$(document).on('click','.add-erp-account',function(){
		...
		$('#employee-erp-content').html('' +
            ...
            '</div>'+
            '');
		var path = "{{route('employee-erp-create')}}?id="+id;
	...
	$(document).on('change','select[name="position_id"]',function(){
		var id = $(this).val();
		if(id==8||id==7){
			$.post("{{ route('employee-functions', ['id' => 'get-team']) }}",
			{id: id},
			function(data){
