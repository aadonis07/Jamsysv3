                ...
                <div class="row p-3">
                    @if($payment_request->status == 'PENDING')
                        <div class="col-md-8 text-danger">
                            <p>* Note: Payment stats is ON <b>PENDING</b>. Modification in this request is enabled.   </p>
                        </div>
...
{{--                                                    <th width="3%">Action</th>--}}
                    ...
                    <div class="col-md-6 text-right">
                        <form method="post"  id="update-payment-status-form" onsubmit="$('#update-payment-status-btn').attr('disabled',true)" id="update-payment-request-status" action="{{ route('payment-request-functions',['id' => 'update-payment-request-status']) }}">
                            @csrf()
                            <input type="hidden" id="update-status" name="status" value="FOR-APPROVAL"/>
        ...
        function updateRequest(){
            ...
            confirm_message('Update Payment Request','Are you sure you want to  update this request ?' , function (confirmed) {
                    if(confirmed) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{route('payment-request-functions',['id' => 'update-payment-request'])}}",
                            data: formData,
                            contentType: false,
                            processData: false,
                            success: function (result) {
