...
@section('content')
    ...
    @endphp
    <form class="" role="form" id="create-product-fitout" onsubmit="$('#btn-add-fitout').attr('disabled',true)" method="POST" action="{{ route('product-functions',['id' => 'add-product-fitout']) }}" enctype="multipart/form-data">
        @csrf()
...
@section('scripts')
    ...
    <script>
        ...
        function addTofitoutItem(btn){
            ...
            $.ajax({
                type: "POST",
                url: "{{route('product-functions',['id' => 'add-to-fit-out-list'])}}",
                data: formData,
                contentType: false,
                processData: false,
                success: function (result) {
