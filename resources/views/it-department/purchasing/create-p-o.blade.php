    ...
    <div class="row mb-3 ">
        <div class="col-lg-12 d-flex flex-start w-100">
            ...
            <div class="d-flex flex-fill">
                <div class="col-md-7">
                    <div class="flex-fill">
                        <span class="h5 mt-0">[ <text class="text-info">Purchase Order</text> ] {{ $supplier->name }} </span>
                        ...
                        <div class=" mb-3 alert alert-primary alert-dismissible">
                            <div class="row">
                                ...
                                <div class="col-md-3 text-right">
                                    ...
                                    <p class="mb-0"><i class="text-primary">{{ $user->employee->first_name }} {{ $user->employee->last_name }}</i></p>
                                ...
                                @foreach($cart as $product)
                                    ...
                                    <tr id="row-{{ $product->id }}">
                                        ...
                                        <td>
                                            ...
                                            <hr class="m-0">
                                            <text class="text-primary">Date Created: [ SAVE ITEM FIRST ]</text>
                                            <hr class="m-0">
                                        ...
                                        <td>
                                            ...
                                            @if($product->attributes['status'] == 'ACTIVE')
                                                ...
                                                <b class="text-primary">TOTAL: {{ number_format($product->quantity) }} </b>
                                            ...
                                            @else
                                                <b class="text-primary">TOTAL: 0 </b>
                                                <button onClick="addQtyModal('{{ $product->id }}')" class="btn btn-xs btn-primary float-right ">Add</button>
                                                <hr class="m-0 mt-2">
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0">
                    ...
                    <form method="post" id="remove-qty-product-form" action="{{ route('purchasing-functions',['id' => 'remove-qty-product']) }}">
                        @csrf()
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <ul class="nav nav-tabs " role="tablist">
                                ...
                                <li class="nav-item  "><a class="nav-link fs-sm text-dark" data-toggle="tab" href="#raw-section" role="tab">RAW / SPECIAL ITEM PRODUCTS</a></li>
                            </ul>
                        </div>
                        <div class="col-md-12">
                ...
                <div class="modal-body pt-0">
                    <form method="post" id="add-qty-product-form" onSubmit="$('#add-btn-qty').attr('disabled',true)" action="{{ route('purchasing-functions',['id' => 'add-qty-product']) }}" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12 mt-3" id="quotation-section">
                            ...
                            <p class="text-danger mt-0 mb-2 bor">*Note: Please select quotation product for qty reference.</p>
    ...
    <script>
        ...
        function supplyProducts(){ //supplier's product
            ...
            $('#supply-product-table').DataTable({
                ...
                "ajax":{
                    url :"{{ route('purchasing-functions',['id' => 'supplier-supply-products']) }}", // json datasource
        ...
        function rawProducts(){ //supplier's product
            ...
            $('#raw-product-table').DataTable({
                ...
                "ajax":{
                    url :"{{ route('purchasing-functions',['id' => 'supplier-raw-products']) }}", // json datasource
                    type: "POST",  // method  , by default get
                    data : { supplier: "{{ $enc_supplier_id }}" },
                    error: function(result){  // error handling
                        $('#err').html(JSON.stringify(result));
        ...
        function removeProduct(key,btn,mode){
            ...
            confirm_message('Remove Item','Are you sure you want to  remove this item ?' , function (confirmed) {
                if(confirmed){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-functions',['id' => 'remove-product-to-cart'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        success: function (result) {
                            if(result.success == 0){
                                alert_message('Remove Item Failed',result.message,'danger');
                            }else{
                                alert_message('Remove Item Success',result.message,'success');
                                $('#row-'+key).remove();
                        ...
                        error: function(result){
                            alert_message('Assign Shopper Failed',result.responseText,'danger');
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        }
                    });
        ...
        function addPOProduct(key,btnClass){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('purchasing-functions',['id' => 'add-po-product']) }}",
                data: formData,
                contentType: false,
                processData: false,
                success: function (result) {
        ...
        function savePO(btn){
            ...
            confirm_message('Saving Item','Are you sure you want to  save this item/s ?' , function (confirmed) {
                if(confirmed){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-functions',['id' => 'save-p-o'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        success: function (result) {
                            if(result.success == 0){
                                ...
                                window.location.replace("{{ route('supplier-update-p-o',['sid'=> $enc_supplier_id]) }}");
                            }
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        },
        ...
        function onProcessQuotations(){
            ...
            $("#quotations").select2({
                ajax:{
                    type: "POST",
                    url: "{{ route('purchasing-functions',['id' => 'approved-accounting-quotations']) }}",
                    //dataType: 'json',
                    delay: 250,
