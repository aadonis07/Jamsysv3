 ...
 $(function(){
	 function move_table(){
		$('#dt-quotation-moved').DataTable({
			...
			"ajax":{
				url :"{{ route('proprietor-quotation-functions',['id' => 'quotation-moved-serverside']) }}",
				type: "POST",  
			...
			columns: [
				...
				{ data: 'sales_agent.employee.first_name', name: 'sales_agent.employee.first_name'},
	 ...
	 function pending_table(){
		$('#dt-quotation-pending').DataTable({
			...
			"lengthMenu": [[50, 100, 150, 250], [50, 100, 150, 250]],
			"ajax":{
				url :"{{ route('proprietor-quotation-functions',['id' => 'quotation-list-serverside']) }}",
				type: "POST",  
				data: {status:"PENDING"},
				"processing": true,
				"serverSide": true,
	 ...
	 function approve_proprietor_table(){
	 ...
	 function approved_accounting_table(){
	 ...
	 move_table();
	 pending_table();
	 approve_proprietor_table();
	 approved_accounting_table();
	 on_process_table();

	 function for_moved(){
		$("#dt-quotation-moved").dataTable().fnDestroy();
	 }
	 function for_pending(){
		$("#dt-quotation-pending").dataTable().fnDestroy();
	 }
	 function for_approved_proprietor(){
		$("#dt-quotation-approved-proprietor").dataTable().fnDestroy();
	 }
	 function for_approved_accounting(){
		$("#dt-quotation-approved-accounting").dataTable().fnDestroy();
	 }
	 function for_on_process(){
		$("#dt-quotation-on-process").dataTable().fnDestroy();
	 }
	...
	$(document).on('click','.approved-quotation',function(){
		...
		Swal.fire({
			...
			if (result.value) {
				$.post("{{ route('proprietor-quotation-functions', ['id' => 'action-quotation']) }}",
				{id: id,qstatus:'APPROVED-PROPRIETOR'},
				function(data){
					...
					if(status=='FOR-APPROVAL'){
						...
						for_approved_accounting();
						approved_accounting_table();
					}else{
						for_on_process();
						on_process_table();
					}
					for_approved_proprietor();
					approve_proprietor_table();
				});
			}else{
	...
	$(document).on('click','.reject-quotation',function(){
		var id = $(this).data('id');
		var status = $(this).data('status');
		$('.reject-quotation').prop('disabled', true);
		Swal.fire({
			title: 'Confirm Save',
			...
			confirmButtonText: 'Yes!, REJECT this quotation.'
		}).then((result) => {
			if (result.value) {
				$.post("{{ route('proprietor-quotation-functions', ['id' => 'action-quotation']) }}",
				{id: id,qstatus:'REJECTED'},
				function(data){
					toastMessage('Success','Your quotation in on '+data+'.','success','toast-top-right');
					if(status=='FOR-APPROVAL'){
						for_moved();
						...
						for_pending();
						pending_table();
					}else if(status=='APPROVED-PROPRIETOR'){
						for_approved_proprietor();
						approve_proprietor_table();
					}else if(status=='APPROVED-ACCOUNTING'){
						for_approved_accounting();
				...
				$('.reject-quotation').prop('disabled', false);
			}
		});
	});
	$(document).on('click','.hold-quotation',function(){
		var id = $(this).data('id');
		var status = $(this).data('status');
		$('.hold-quotation').prop('disabled', true);
		Swal.fire({
			title: 'Confirm Save',
			text: "Are you sure you want to HOLD this quotation ?",
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#4a4646',
			confirmButtonText: 'Yes!, HOLD this quotation.'
		}).then((result) => {
			if (result.value) {
				$.post("{{ route('proprietor-quotation-functions', ['id' => 'action-quotation']) }}",
				{id: id,qstatus:'HOLD'},
				function(data){
					toastMessage('Success','Your quotation in on HOLD.','success','toast-top-right');
					if(status=='FOR-APPROVAL'){
						for_moved();
						move_table();
					}else if(status=='PENDING'){
