...
@endsection
@section('breadcrumbs')
    <li class="breadcrumb-item "><a href="{{ route('purchasing-raw-list') }}">Purchasing</a></li>
    <li  class="breadcrumb-item "> <a href="{{ route('purchasing-raw-supplier-list') }}">Suppliers</a></li>
    ...
    <div class="row mb-3 ">
        <div class="col-lg-12 d-flex flex-start w-100">
            ...
            <div class="d-flex flex-fill">
                <div class="col-md-7">
                    <div class="flex-fill">
                        <span class="h5 mt-0">[ <text class="text-dark">Purchase Order</text> ] <text class="text-muted">{{ $supplier->name }}</text> </span>
                        <p class="mb-0">Please select supplier's product</p>
                    </div>
                </div>
            ...
            <div id="panel-1" class="panel">
                <div class="row p-3">
                        ...
                        <div class=" mb-3 alert alert-primary alert-dismissible">
                            <div class="row">
                                ...
                                <div class="col-md-3 text-right">
                                    <p class="mb-0 ">P.O #: <text class="text-danger">[ After Save ]</text></p>
                                    <p class="mb-0">Date Created: <text class="text-danger">[ After Save ]</text></p>
                                    <p class="mb-0">Prepare By: </p>
                                    <p class="mb-0"><i class="text-dark">{{ $user->employee->first_name }} {{ $user->employee->last_name }}</i></p>
                                ...
                                @foreach($cart as $product)
                                    ...
                                    <tr id="row-{{ $product->id }}">
                                        ...
                                        <td>
                                            ...
                                            <hr class="m-0">
                                            <text class="text-dark">Date Created: [ SAVE ITEM FIRST ]</text>
                                            <hr class="m-0">
                                                    ...
                                                    @php
                                                        ...
                                                        if($type == 'QUOTATION'){
                                                                $htmlDetail .= '<li class="small" title="'.$type.'">
                                                                        [ <a onClick=updateQty("'.$temp_key.'") href="javascript:;">'.$detail['qty'].' qty</a> ] '.$detail['name'].'
                                                                            <ul>
                                        ...
                                        <td>
                                            <form method="post" id="update-price-product-form-{{ $product->id }}" action="{{ route('purchasing-raw-purchasing-functions',['id' => 'update-price-product']) }}">
                                                @csrf()
                                                <input type="hidden" name="supplier_product_key" value="{{ $product->id }}">
                                                <input type="hidden" name="supplier_key" value="{{ $enc_supplier_id }}">
                                                <div class="form-group">
                                        ...
                                        <td>
                                            ...
                                            @if($product->attributes['status'] == 'ACTIVE')
                                                <div class="custom-control custom-checkbox custom-control-inline">
                                                    <input type="checkbox" value="{{ $product->id }}" name="is_save[]" class="custom-control-input" id="defaultInline{{ $product->id }}">
                                                    <label class="custom-control-label" for="defaultInline{{ $product->id }}">Save</label>
                                                </div>
                                            @endif
                                        </td>
                                    </tr>
                    ...
                    <div class="col-md-9 text-right">
                        <p class="text-danger">*Note: Saving this p.o will generate p.o number and EWT fields</p>
                    </div>
                    <div class="col-md-3 text-right">
                        <div class="form-group">
                            <button onclick="savePO('btn-save-items')"  class="btn btn-save-items btn-info btn-sm">Save checked Items</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0">
                    <form method="post" id="update-qty-product-form" action="{{ route('purchasing-raw-purchasing-functions',['id' => 'update-qty-product']) }}">
                        @csrf()
                        <div class="row update-qty-detail">

                        </div>
                    </form>
                    <form method="post" id="remove-qty-product-form" action="{{ route('purchasing-raw-purchasing-functions',['id' => 'remove-qty-product']) }}">
                        @csrf()
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <ul class="nav nav-tabs " role="tablist">
                                <li class="nav-item  "><a class="nav-link active fs-sm text-dark" data-toggle="tab" href="#raw-section" role="tab">RAW / SPECIAL ITEM PRODUCTS</a></li>
                            </ul>
                        </div>
                        <div class="col-md-12">
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="raw-section" role="tabpanel">
                                    <div class="row">
    ...
    <div class="modal fade" id="add-qty-modal" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body pt-0">
                    <form method="post" id="add-qty-product-form" onSubmit="$('#add-btn-qty').attr('disabled',true)" action="{{ route('purchasing-raw-purchasing-functions',['id' => 'add-qty-product']) }}" enctype="multipart/form-data">
                        @csrf()
                        <div class="row mt-0">
                        ...
                        <div class="col-md-12 mt-1" id="department-section" style="display:none">
                            <div class="form-group">
                                ...
                                <select class="form-control departments-details" name="department" id="department">
...
@section('scripts')
    ...
    <script>
        ...
        function rawProducts(){ //supplier's product
            $('#raw-product-table').dataTable().fnDestroy();
            $('#raw-product-table').DataTable({
                "pageLength": 50,
                "processing": true,
                "serverSide": true,
                "lengthMenu": [[50, 100, 150, 250], [50, 100, 150, 250]],
                "ajax":{
                    url :"{{ route('purchasing-raw-purchasing-functions',['id' => 'supplier-raw-products']) }}", // json datasource
                ...
                columns: [
                    ...
                    { data: 'code', name: 'code'},
                    { data: 'product.product_name', name: 'product.product_name',visible: false},
        ...
        function removeProduct(key,btn,mode){
            ...
            confirm_message('Remove Item','Are you sure you want to  remove this item ?' , function (confirmed) {
                if(confirmed){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-raw-purchasing-functions',['id' => 'remove-product-to-cart'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        ...
                        error: function(result){
                            alert_message('Assign Shopper Failed',result.responseText,'DIALOG','danger');
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        }
                    });
        ...
        function addPOProduct(key,btnClass){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('purchasing-raw-purchasing-functions',['id' => 'add-po-product']) }}",
                data: formData,
                contentType: false,
                processData: false,
                success: function (result) {
        ...
        function savePO(btn){
            $('.'+btn).attr('disabled',true);
            confirm_message('Saving Item','Are you sure you want to  save this item/s ?' , function (confirmed) {
                if(confirmed){
                    toastMessage('Saving Items','Saving PO item in the list..','info','toast-bottom-right');
                    formData = new FormData();
                    formData.append('supplier', '{{ $enc_supplier_id }}');
                    $('input[name^="is_save"]').each(function() {
                        if($(this).is(":checked") === true) {
                            formData.append('supplier_products[]', $(this).val());
                        }
                    });
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-raw-purchasing-functions',['id' => 'save-p-o'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        success: function (result) {
                            if(result.success == 0){
                                alert_message('Save PO Failed',result.message,'DIALOG','danger');
                            }else{
                                alert_message('Save PO Success',result.message,'DIALOG','success');
                                window.location.replace("{{ route('purchasing-raw-supplier-update-p-o',['sid'=> $enc_supplier_id]) }}");
                            }
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        },
                        error: function(result){
                            alert_message('Save PO Failed',result.responseText,'DIALOG','danger');
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        }
                    });
        ...
        function onProcessQuotations(){
            ...
            $("#quotations").select2({
                ajax:{
                    type: "POST",
                    url: "{{ route('purchasing-raw-purchasing-functions',['id' => 'approved-accounting-quotations']) }}",
                    //dataType: 'json',
                    delay: 250,
        ...
        function quotationProducts(quotation){
            ...
            $('#quotation-products-table').DataTable({
                ...
                "ajax":{
                    url :"{{ route('purchasing-raw-purchasing-functions',['id' => 'quotation-products']) }}", // json datasource
                    type: "POST",  // method  , by default get
