    ...
    <script>
        ...
        function releasedPr(){ // for liquidation
            $('#released-tbl').DataTable({
                ...
                "responsive": true,
                "ajax":{
                    url :"{{route('purchasing-supply-liquidation-functions',['id' => 'released-payment-request-list'])}}", // json datasource
        ...
        function liquidatingPr(){ // liquidating
            $('#liquidating-tbl').DataTable({
                ...
                "responsive": true,
                "ajax":{
                    url :"{{route('purchasing-supply-liquidation-functions',['id' => 'liquidating-payment-request-list'])}}", // json datasource
                    type: "POST",  // method  , by default get
                    error: function(data){  // error handling
                        $('#err').html(JSON.stringify(data));
                    }
        ...
        function liquidatedPr(){ // liquidated
            $('#liquidated-tbl').DataTable({
                ...
                "responsive": true,
                "ajax":{
                    url :"{{route('purchasing-supply-liquidation-functions',['id' => 'liquidated-payment-request-list'])}}", // json datasource
                    type: "POST",  // method  , by default get
                    error: function(data){  // error handling
                        $('#err').html(JSON.stringify(data));
                    }
        ...
        function liquidate(payment_request_key){
            ...
            var url = "{{ route('purchasing-supply-liquidation-pr-details') }}";
        ...
        function payees(){
            $("#payee").select2({
                ajax:{
                    type: "POST",
                    url: "{{ route('purchasing-supply-liquidation-functions',['id' => 'payee-list']) }}",
                    //dataType: 'json',
                    delay: 250,
        ...
        function createLiquidation(btn){
            ...
            confirm_message('Liquidate','Are you sure you want to  liquidate this details ?' , function (confirmed) {
                if(confirmed) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-supply-liquidation-functions',['id' => 'add-to-liquidation'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
        ...
        function showLiquidated(payment_request){
            $('#liquidated-table').DataTable({
                ...
                "responsive": true,
                "ajax":{
                    url :"{{route('purchasing-supply-liquidation-functions',['id' => 'liquidation-list'])}}", // json datasource
        ...
        function removeLiquidated(key,btn){
            ...
            confirm_message('Remove Liquidated data','Are you sure you want to remove this details ?' , function (confirmed) {
                if(confirmed) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-supply-liquidation-functions',['id' => 'remove-to-liquidation'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        success: function (result) {
