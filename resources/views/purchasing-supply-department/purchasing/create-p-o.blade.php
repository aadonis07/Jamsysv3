...
@section('scripts')
    ...
    <script>
        ...
        {{--        "ajax":{--}}
        {{--            url :"{{ route('purchasing-supply-purchasing-functions',['id' => 'supplier-raw-products']) }}", // json datasource--}}
        {{--            type: "POST",  // method  , by default get--}}
        {{--            data : { supplier: "{{ $enc_supplier_id }}" },--}}
        {{--            error: function(result){  // error handling--}}
        {{--                $('#err').html(JSON.stringify(result));--}}
        ...
        function savePO(btn){
            ...
            confirm_message('Saving Item','Are you sure you want to  save this item/s ?' , function (confirmed) {
                if(confirmed){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{route('purchasing-supply-purchasing-functions',['id' => 'save-p-o'])}}",
                        data: formData,
                        contentType: false,
                        processData: false,
                        success: function (result) {
                            if(result.success == 0){
                                ...
                                window.location.replace("{{ route('purchasing-supply-supplier-update-p-o',['sid'=> $enc_supplier_id]) }}");
                            }
                            $('.'+btn).attr('disabled',false);
                            toastr.clear();
                        },
