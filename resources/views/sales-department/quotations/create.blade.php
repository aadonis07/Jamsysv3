						...
						<div class="accordion" id="accordionExample">
							<div class="card mb-2 border-bottom">
								...
								<div id="collapseOne" class="collapse {{$qcollapse}}" aria-labelledby="headingOne" data-parent="#accordionExample" style="">
									<div class="card-body">
									<form method="post" id="quotation-information-form" onsubmit="qInfoBtn.disabled = true;" action="{{ route('sales-quotation-functions', ['id' => 'quotation-information']) }}">
                        				@csrf()
							...
							<div class="card mb-2" {{$bactive}}>
								...
								<div id="collapseTwo" class="collapse {{$bcollapse}}" aria-labelledby="headingTwo" data-parent="#accordionExample" style="">
									<div class="card-body">
										<form method="post" id="delivery-information-form" onsubmit="dInfoBtn.disabled = true;" action="{{ route('sales-quotation-functions', ['id' => 'delivery-information']) }}">
											@csrf()
							...
							<form method="post" id="save-quotation-form" action="{{ route('sales-quotation-functions', ['id' => 'save-quotation']) }}">
								@csrf()
...
<div class="modal fade"  id="add-product-modal" role="dialog" >
	<div class="modal-dialog modal-dialog-centered modal-lg">
		<div class="modal-content">
			...
			<form id="add-quotation-product-form" method="POST" onsubmit="qProdBtn.disabled = true;" action="{{ route('sales-quotation-functions',['id' => 'create-quotation-product']) }}" enctype="multipart/form-data">
                @csrf()
...
$(document).ready(function(index){
	...
	if("{{$product_availability}}"=='true'){
		var product_path = "{{route('sales-quotation-products')}}";
		$('#page_list').load(product_path,function(){
			...
			$.post("{{ route('sales-quotation-functions', ['id' => 'quotation-amount-info']) }}",
						...
						if($.trim(final_commission)){
							...
							$('#submitRequestBtn').prop('disabled', true);
								Swal.fire({
									...
									if (result.value) {
										var path = "{{route('sales-quotation-preview')}}";
        ...
        update  : function(event, ui) {
          ...
          var id_array = new Array();
			 ...
			 $(document).on('click','#save-arrangement',function(){
				...
				$.post("{{ route('sales-quotation-functions', ['id' => 'save-arrangement']) }}",
				{id_array: id_array,},
				function(success){
	...
	$(document).on('change','input[name="discountprod[]"]',function(){
		...
		function(data){
			...
			$.post("{{ route('sales-quotation-functions', ['id' => 'quotation-products-temporary']) }}",
			{	
				order: order,
	...
	$(document).on('click','.cancelBtn',function(){
		Swal.fire({
			...
			if (result.value) {
				var id = 1;
				$.post("{{ route('sales-quotation-functions', ['id' => 'quotation-cancel']) }}",
				{	
					id:id
				},
				function(data_new){
