...
<!-- =================================================================================================================================== -->
<form method="post" id="quotation-update-form" action="{{ route('sales-quotation-functions', ['id' => 'quotation-update']) }}">
@csrf()
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    ...
                    @php 
                        $commi_content = '';
                        if(empty($quotation->request_commission)){
                            $commi_content = 'style=display:none';
                        }
                    @endphp
                    @if(!empty($quotation->is_requested_si))
                    ...
                    @endif
                    <div class="form-group commission_content" {{$commi_content}}>
                        <label>Contract Amount</label>
                        <div class="input-group mar-btm">
                            <span class="input-group-btn">
                                <button class="btn btn-default" disabled> Php</button> 
                            </span> 
                            <input class="form-control bg-white" name="contract-amount" value="{{number_format((float)$quotation->grand_total,2, '.', '')}}"  onkeypress="return isNumberKey(event)" readonly />
                        </div>
                    </div>
                    <div class="form-group commission_content" {{$commi_content}}>
                        <label>Commission Requested</label>
                        <input class="form-control" name="commission-amount" value="{{number_format((float)$quotation->request_commission,2, '.', '')}}"/>
                    </div>
                    <div class="form-group commission_content" {{$commi_content}}>
                    ...
                    <div class="form-group commission_content" {{$commi_content}}>
                        ...
                        <textarea class="form-control" row="5" readonly  name="solution_commi">{{$quotation->commission_formula}}</textarea>
                    </div> 
                    <div class="form-group commission_content" {{$commi_content}}>
...
<div class="modal fade"  id="add-product-modal" role="dialog" >
	<div class="modal-dialog modal-dialog-centered modal-lg">
		<div class="modal-content">
			...
			<form id="add-quotation-product-form" method="POST" onsubmit="qProdBtn.disabled = true;" action="{{ route('sales-quotation-functions',['id' => 'update-quotationProduct']) }}" enctype="multipart/form-data">
                @csrf()
...
<!-- ======================================================================================================= -->
<form method="post" id="quotation-clear-form" action="{{ route('sales-quotation-functions', ['id' => 'quotation-clear']) }}">
@csrf()
...
$(document).ready(function(index){
    var warranty = $('#warranty').text();
    if(warranty=='No'){
        $('.warranty-content').css("display", "none");
    ...
    $('select[name="warranty"]').val(warranty).trigger('change');
    var terms_temp = document.getElementById('terms_condition').innerHTML;
    var new_temp = terms_temp;
    $('input[name="terms_condition"]').val(new_temp);
    $('#dt-quotation-products').DataTable({
        ...
        "ajax":{
            url :"{{ route('sales-quotation-functions',['id' => 'quotation-products-serverside']) }}",
            type: "POST",  
    ...
    $(document).on('click','#add-new-product',function(){
		...
		$('#dt-products').DataTable({
			...
			"serverSide": true,
			"ajax":{
				url :"{{ route('sales-quotation-functions',['id' => 'product-list-serverside']) }}",
				type: "POST",  
				data : {nature:nature},
				"pageLength": 100,
				"processing": true,
    ...
    $(document).on('change','select[name="select-region"]',function(){
		var id = $(this).val();
		$.post("{{ route('sales-supplier-functions', ['id' => 'fetch-provinces']) }}",
		{id: id,},
		function(data){
	...
	$(document).on('change','select[name="select-province"]',function(){
		var id = $(this).val();
		$.post("{{ route('sales-supplier-functions', ['id' => 'fetch-cities']) }}",
		{id: id,},
		function(data){
	...
	$(document).on('change','select[name="city-content"]',function(){
		var id = $(this).val();
		$.post("{{ route('sales-quotation-functions', ['id' => 'fetch-barangays']) }}",
		{id: id,},
		function(data){
    ...
    $(document).on('change','select[name="branch"]',function(){
		...
		function(data){
			...
			$.post("{{ route('sales-quotation-functions', ['id' => 'get-address']) }}",
    ...
    $(document).on('change','select[name="client"]',function(){
		var id = $(this).val();
		$.post("{{ route('sales-quotation-functions', ['id' => 'get-branches']) }}",
		{id: id,},
		function(data){
    ...
    $(document).on('click','.product',function(){
		...
		var id = $(this).data('id');
		$.post("{{ route('sales-quotation-functions', ['id' => 'fetch-product-details']) }}",
		{id: id,},
		function(data){
	...
	$(document).on('change','select[name="swatch"]',function(){
		var id = $(this).val();
		$.post("{{ route('sales-quotation-functions', ['id' => 'fetch-swatch-details']) }}",
		{id: id,},
		function(data){
	...
	$(document).on('click','input[name="variant"]',function(){
		...
		$.post("{{ route('sales-quotation-functions', ['id' => 'fetch-variant-details']) }}",
		{id: id,},
		function(data){
			$('input[name="view_amount"]').val(data.display_price);
			$('input[name="product_amount"]').val(data.price);
	...
	$(document).on('click','.add-variant',function(){
		...
		$('#update'+id).html('<td colspan="2">'+
							...
							'</div>'
							+'</td>');
			$.post("{{ route('sales-quotation-functions', ['id' => 'fetch_attribute']) }}",
            {
                id: category,
            },
            function(data){
		...
		if($.trim(attribute)){
			if($.trim(attribute_value)){
				...
				if($.trim(price)){
					...
					$.post("{{ route('sales-quotation-functions', ['id' => 'create-attribute']) }}",
					{
						attribute: attribute,
    ...
    $(document).on('click','.cancel-product-reason',function(){
        ...
        $('#on-process-content').html('' +
            ...
            '</div>'+
            '');
        var path = '{{route("sales-quotation-product-process")}}?id='+id;
    ...
    $(document).on('click','.cancel-product',function(){
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.post("{{ route('sales-quotation-functions', ['id' => 'delete-product-update']) }}",
    ...
    $(document).on('click','.revert-product',function(){
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.post("{{ route('sales-quotation-functions', ['id' => 'revert-product-update']) }}",
                {id: id,quote_number:quote_number,sub_total:sub_total,installation_charge:installation_charge,delivery_charge:delivery_charge,discount_product_quotation:discount_product_quotation,discount_quotation:discount_quotation,total_discount:total_discount,grand_total:grand_total},
                function(data){
    ...
    $(document).on('change','select[name="commi-type"]',function(){
		...
		if($.trim(amount_requested)){
			...
			if($.trim(commi_type)){
				...
				$.post("{{ route('sales-quotation-functions', ['id' => 'compute-commission']) }}",
