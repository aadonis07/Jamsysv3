...
<div class="modal fade" id="activate-modal">
	<div class="modal-dialog modal-sm modal-dialog-centered" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" id="control-form"  action="{{ route('user-functions', ['id' => 'control-account']) }}">
					@csrf()
...
<script>
 $(function(){
		$('#dt-users-active').DataTable( {
                ...
                "serverSide": true,
                "ajax":{
                    url :"{{ route('user-functions',['id' => 'user-active-serverside']) }}",
                    type: "POST",  
					data: {status:"ACTIVE"},
					"pageLength": 100,
                   "processing": true,
...
$(document).ready(function(index){
	...
	$(document).on('click','.action-btn',function(){
		$('#account-content').html('' +
            ...
            '');
		var id = $(this).data('id');
		var path = "{{route('user-control')}}?id="+id;
